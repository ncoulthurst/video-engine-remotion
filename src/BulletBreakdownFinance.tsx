/**
 * BulletBreakdownFinance — Generic titled argument / breakdown card,
 * pinned to the light paper-orange finance palette (warm-white + burnt orange).
 *
 ...
 * Fork of BulletBreakdown repinned to PALETTES.paperOrange — orange is the
 * single signal (kicker rule, index numbers); all text stays ink/muted.
 *
 * Stack: Bg (0) → Vignette → Content (10) → Grain (last).
 */
...
// Light background → depth is a FAINT warm edge vignette, never heavy black.
const VIGNETTE =
...
export const BulletBreakdownFinancePropsSchema = z.object({
...
export type BulletBreakdownFinanceProps = z.infer<typeof BulletBreakdownFinancePropsSchema>;

const PADDING_X = 200;

export const BulletBreakdownFinance: React.FC<BulletBreakdownFinanceProps> = ({
  title,
  subtitle,
  points,
  accent,
  ...
  // Palette prop is kept for schema compatibility but this comp is pinned
  // to paperOrange: ink text, muted secondary, hairline dividers, one orange.
  const fg      = P.ink;
  const muted   = P.muted;
  const divider = P.line;
  ...
  return (
    <AbsoluteFill style={{ fontFamily, background: P.bg }}>
      ...
      <AbsoluteFill style={{ zIndex: 10, padding: `120px ${PADDING_X}px` }}>
        ...
        <div style={{ opacity: titleP, transform: `translateY(${titleY}px)` }}>
          ...
          <div style={{
            fontFamily, fontWeight: 700, fontSize: 72,
        ...
        <div style={{ position: "absolute", left: PADDING_X, right: PADDING_X, top: startY }}>
          {rows.map((p, i) => {
            ...
            return (
              ...
              >
                <div style={{
                  flexShrink: 0, fontFamily: geistMonoFamily, fontWeight: 700,
      ...
      <div style={{ position: "absolute", inset: 0, zIndex: 20, pointerEvents: "none", opacity: 0.35 }}>
        <Grain />
      </div>
    </AbsoluteFill>
  );
...
export default BulletBreakdownFinance;
