 ...
 * F3: composed from the shared kit (Ground/prog) — the per-frame SVG turbulence
 * grain is gone (Ground/FilmPlate own texture now). Dots accept the "entity"
 * beat (H1).
 */
...
import { Ground, TYPE, EASE, prog, beatDelay, stagger } from "./lib/kit";
...
export const HeroScatterPlot: React.FC<HeroScatterPlotProps> = ({ axisXLabel, axisYLabel, q1Label, q2Label, q3Label, q4Label, showWizardArrow, players, bgColor, skipIntro = false, beats }) => {
  const frame = useCurrentFrame(); const { fps, width, height } = useVideoConfig();
  const axisProgress = skipIntro ? 1 : prog(frame, 0, 26, EASE.soft);
  const dotStart = beatDelay(beats, "entity", fps, 28);
  const playerProgs = players.map((_, i) => (skipIntro ? 1 : prog(frame, dotStart + stagger(i, 8), 16, EASE.snap)));
  ...
  return (
    <Ground ground="ink" bgColor={bgColor} domain="football" texture skipIntro={skipIntro} pad={0}>
      <svg width={width} height={height} style={{ position: "absolute", top: 0, left: 0, pointerEvents: "none" }}>
        ...
        <text x={PADDING + axisW - 8} y={cy + 28} fill="rgba(255,255,255,0.28)" fontSize={13} fontFamily={TYPE.serif} fontStyle="italic" textAnchor="end" opacity={axisProgress}>{axisXLabel}</text>
        <text x={cx + 12} y={PADDING + 18} fill="rgba(255,255,255,0.28)" fontSize={13} fontFamily={TYPE.serif} fontStyle="italic" opacity={axisProgress}>{axisYLabel}</text>
      </svg>
      {[ { label: q2Label, x: PADDING + 30, y: PADDING + 50, rotate: -20 }, { label: q1Label, x: width - PADDING - 30, y: PADDING + 50, rotate: 20, anchor: "end" }, { label: q3Label, x: PADDING + 30, y: height - PADDING - 30, rotate: 20 }, { label: q4Label, x: width - PADDING - 30, y: height - PADDING - 30, rotate: -20, anchor: "end" } ].map((q, i) => (
        <div key={i} style={{ position: "absolute", left: q.x, top: q.y, fontFamily: TYPE.serif, fontSize: 20, fontStyle: "italic", fontWeight: 400, color: "rgba(255,255,255,0.28)", transform: `translate(-50%, -50%) rotate(${q.rotate}deg)`, opacity: axisProgress, whiteSpace: "nowrap", textAlign: q.anchor === "end" ? "right" : "left" }}>{q.label}</div>
      ))}
      {players.map((player, i) => (
        <div key={i} style={{ position: "absolute", left: plotX(player.x) - 31, top: plotY(player.y) - 31, width: 62, height: 62, opacity: playerProgs[i], transform: `scale(${interpolate(playerProgs[i], [0, 1], [0.3, 1])})` }}>
