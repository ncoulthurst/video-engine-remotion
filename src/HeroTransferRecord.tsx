 ...
 * REFERENCE VERSION — pinned to the warm `editorial` (cream) palette. Kept frozen
 * as the canonical look. The dark Vault (green + red) variant is a true copy in
 * HeroTransferRecordVault.tsx — edit that one, not this one.
 */
...
// This comp is pinned to the cream editorial palette (the reference look).
const PAL: Palette = PALETTES.editorial;

// Portrait CSS mask — mirrors HeroSeasonTimeline: fade transparent→solid from the
// left so the image dissolves into the page; the mask handles the blending.
...
// Colours derived from the pinned palette. `bgColor`/`accentColor` props and
// per-row `accentColor` still override.
const INK       = PAL.ink;
...
// Dark vs light background → decide portrait treatment (duotone vs grayscale).
const BG_LUM    = (() => { const [r, g, b] = hexToRgb(PAL.bg); return (0.299 * r + 0.587 * g + 0.114 * b) / 255; })();
const IS_DARK   = BG_LUM < 0.4;

export const HeroTransferRecord: React.FC<HeroTransferRecordProps> = ({ title, titleSize = 74, subtitle, sideImage, accentColor, transfers, bgColor, skipIntro = false }) => {
  ...
  // The single accent + background: prop overrides, else the pinned palette.
    ...
    <AbsoluteFill>
      ...
      {sideImage && (
        <div style={{ position: "absolute", right: 0, top: 0, width: 900, height: "100%", opacity: interpolate(imageProg, [0, 1], [0, IS_DARK ? 0.92 : 0.88]), transform: `translateX(${interpolate(imageProg, [0, 1], [80, 0])}px)`, WebkitMaskImage: PORTRAIT_MASK, maskImage: PORTRAIT_MASK, zIndex: 1 }}>
          {/* Portrait treatment is palette-aware: on a DARK palette, a duotone maps
              the photo into palette tones (fixes light-bg patches). On a LIGHT
              palette (this reference), a crisp grayscale dissolves into the paper. */}
          ...
          {/* Bottom vignette — dissolves into the page colour */}
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 150px", maxWidth: sideImage ? 1180 : 1920, zIndex: 10 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          {transfers.map((t, i) => {
            ...
            // Each row slides up as it fades in.
            ...
            // Accent lives only on the bar and the left tab. Per-row override honoured.
            const rowAccent  = t.accentColor || (featured ? accent : BAR_SOFT);
            const nameColor  = featured ? INK : SECONDARY;
            const valueColor = featured ? INK : FAINT;   // figures stay neutral
            ...
            return (
              <div key={i} style={{ position: "relative", opacity, transform: `translateY(${rowShift}px)`, display: "flex", alignItems: "center", height: ROW_H, borderBottom: i < transfers.length - 1 ? `1px solid ${HAIRLINE}` : "none" }}>
                ...
                {/* bar cell — right-anchored so every bar ends at a common point and leads into its value */}
                ...
                {/* value — left-aligned, sits directly after the bar; Geist Mono for a finance-data feel */}
